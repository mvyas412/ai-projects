...
| 4 | Fine-grained isolation and governance | Central RBAC/ACL, RLS, vector/object enforcement, permission snapshots, and security audit/export implemented; lifecycle follows | PostgreSQL, Qdrant, object storage | In progress — Milestones 4.0–4.4 complete |
...
**Status:** In progress. Milestones 4.0–4.4 are complete. Phase 2 starts isolation;
...
connector permission contract, safe append-only security review, and checksummed
compliance export are implemented; lifecycle components remain milestone work.

The review source is the
[Phase 4 policy matrix and threat model](PHASE4_POLICY_THREAT_MODEL.md). ADRs
0013–0017 are Accepted; Milestone 4.5 is the next implementation slice.
